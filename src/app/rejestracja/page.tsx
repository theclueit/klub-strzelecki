...
import { UserPlus, Users, Zap, Search, Check } from 'lucide-react'
...
export default function RegistrarPage() {
  ...
  useEffect(() => {
    if (member && (member.role === 'registrar' || member.role === 'admin')) loadData()
  }, [member, loadData])

  if (loading) return <div className="p-16 text-center text-muted">Ładowanie...</div>
  if (!member || (member.role !== 'registrar' && member.role !== 'admin')) {
    return (
